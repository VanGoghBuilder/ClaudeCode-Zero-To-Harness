...
这一节把常见开发动作沉淀成可复用的 slash commands。不要把命令理解成“快捷提示词”，更准确的说法是：它们把上下文读取、工具权限、输出格式和验收规则固定下来，让 Claude Code 每次都按同一套流程工作。

#### 从最佳实践提炼出的命令组合

| 命令 | 适合场景 | 核心动作 | 依赖条件 |
| --- | --- | --- | --- |
| `/custom-init` | 新项目接入 Claude Code | 分析目录结构、技术栈和开发模式，生成或更新 `CLAUDE.md` | 项目已有基本目录和构建方式 |
| `/commit` | 提交一组已经完成的变更 | 检查 diff、判断是否需要拆分、生成规范 commit 信息 | Git 仓库干净可读，最好先完成测试 |
| `/issue <number>` | 从 GitHub issue 开始实现需求 | 读取 issue、规划、建分支、实现、验证并准备 PR | 已安装并登录 GitHub CLI |
| `/reviewpr <number>` | 审查 Pull Request | 拉取 PR 信息、检查 CI、审查 diff、输出结构化反馈 | 已安装并登录 GitHub CLI |
| `/test <scope>` | 修复前后验证质量 | 识别测试框架、运行定向测试、分析失败并复跑 | 项目测试命令可在本地执行 |
...
这条路径的重点不是“让 Claude 自己包办一切”，而是把一次需求交付拆成可观察、可验证、可回滚的阶段。每个命令都应该有明确的输入、允许使用的工具、完成标准和失败时的退出方式。
