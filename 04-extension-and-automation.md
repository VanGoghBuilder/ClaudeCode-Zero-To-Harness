...
| 代理类型 | 主要职责 | 适合委托的任务 |
| --- | --- | --- |
| 通用探索代理 | 多步骤分析、文件搜索、跨模块信息收集 | “找出这个功能涉及哪些文件” |
| 架构代理 | 技术栈判断、边界设计、可扩展性评估 | “评估这次重构是否破坏模块边界” |
| 技术写作代理 | 文档整理、API 文档、变更说明 | “把这段实现整理成读者能理解的指南” |
| 全栈开发代理 | 跨数据库、API、前端的端到端实现 | “按计划实现一个小型功能切片” |
| 后端代理 | API、数据库、服务端逻辑、性能问题 | “检查这个查询是否有 N+1 风险” |
| 前端代理 | UI、组件、浏览器验证、交互问题 | “验证这个表单在浏览器中的完整路径” |
| QA 代理 | 测试策略、自动化验证、覆盖率缺口 | “找出这次变更还缺哪些测试” |
| 调试 / 代码质量代理 | 系统化排错、重构建议、代码审查 | “解释这个失败日志并定位根因” |
| 技术负责人代理 | 安全、性能、架构取舍和发布风险 | “上线前独立审查这组改动” |

一个稳定的团队工作流通常不是只靠一个“大而全”的代理，而是让命令按阶段调用不同代理。例如：

- `/custom-init` 更适合调用架构代理、技术写作代理和通用探索代理
- `/commit` 更适合调用代码质量代理或技术负责人代理，检查提交是否原子化
- `/issue` 可以按需要调用全栈、后端、前端、QA 和通用探索代理
- `/reviewpr` 应优先调用代码质量、技术负责人、QA 和架构代理
- `/test` 应优先调用 QA、调试、后端或前端代理
