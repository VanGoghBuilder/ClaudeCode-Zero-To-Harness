...
如果你使用 Claude Desktop，并希望在桌面端的 Claude Cowork 或 Claude Code 中接入第三方 API、企业 LLM Gateway 或兼容 `/v1/messages` 的推理网关，可以参考独立说明：

- [Claude Desktop：通过开发者模式配置第三方 API](09-claude-desktop-third-party-api.md)

这篇会单独说明如何开启 Developer Mode、配置 Gateway endpoint / API key / auth scheme / model list，以及如何使用 `Local configuration` 启动。
